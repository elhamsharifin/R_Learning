...
#probability of not choosing cyan in the second round if the first one is selected (sampling with replacement)
p_not_cyan_with <- (magenta + yellow)/(cyan + magenta + yellow) #with replacement
p_not_cyan_with
...
In SAT exam, every correct answer awarded 1 point and any incorrecy answer has a penalty of -0.25 point. The quantitative test is consisted of 44 multipli-choice questions for which there are 5 answer choices. Assume that a student answer ther question by gussing all questions.

### Using Central Limit Theory:
```{r}
### using CLT:
###  probability of guessing correctly for one question
p_correct <- 1/5 #one correct answer among 5 options
p_correct
p_incorrect <- 4/5
p_incorrect

### expected value of points for gussing correct for one question:
E_correct <- (+1)*(p_correct)+(-0.25)*(p_incorrect) #ap+b(1-p)
E_correct

### expected value of gussing all 44 questions: #n.(ap+b(1-p))
E_all <- ((+1)*(p_correct)+(-0.25)*(p_incorrect))*44 
E_all

### standard error of gussing all questions: sqrt(n)|b-a|sqrt(p(1-p))
se_all <- sqrt(44)*abs(+1--0.25)*sqrt(p_correct*p_incorrect)
se_all

### using CLT, what is the probability of a gussing student be 8 points or more?
1-pnorm(8,E_all,se_all)
```
### Using Monto Carlo Simulation:
```{r}
...
S <- replicate(B,{
  X <- sample(c(-0.25,+1),n,replace = TRUE,prob=c(p_incorrect,p_correct))
  sum(X)
})
...
```
### updating the SAT exam: eliminating the penalty score and changing number of the quesions to 4:
```{r}
...
A casino is offering a Special bet on roulette, that is a bet on five pockets (00, 0, 1, 2, 3) out of 38 total pockets. The bet pays out 6 to 1. In fact, a losing bet is uqual to -1 dollar and a successful bet is equal to 6 dollar. A gambler wants to know the chance of losing money if he places 500 bets on the roulette House Special.
```{r}
...
### the expected value of the average payout over 500 bets?
#Note:  the expected value of the average is differnt from the expected value of the sum
exp_v  

### the standard error of the average payout over 500 bets?
 se /(sqrt(n))    #(σ/√n)
 
 
### the expected value of sum of 500 bets:
 n * exp_v
 
 ### the standard error of sum of 500 bets:
 sqrt(n)*se
     #√nσ
 
 ### what is the probability of losing money over 500 bets?
 # Note: use sexpected value and standard error of sum
 pnorm(0, n * exp_v,sqrt(n)*se)
```

## Key points: The Big Short: Interest Rates Explained
...
```{r}
### suppose a bank give 1000 loans in the total amount of 180,000$ this year. and suppose after adding up all the expenses, bank loss 200,000$ per foreclosure. 


### Code: Interest rate sampling model
n <- 1000
loss_per_closure <- -200000
p <- 0.02 #0.02 of customers are default and they dont pay back the money which they borrowed
defaults <- sample(c(0,1),n,replace=TRUE,prob=c(1-p,p)) #1 shows lose money
sum(defaults*loss_per_closure)
# if we run the code for different times: we will have different answer, because it is the probability and not sure
```
```{r}
### Code: Interest rate Monto Carlo simulation
n <- 1000
B <- 10000
...
### distribution from Monto-Carlo (plotting expected losses):
library(tidyverse)
data.frame(losses_per_milions=losses/10^6) %>% ggplot(aes(losses_per_milions)) + geom_histogram(binwidth=0.6,color="black")
```
actually, we dont need to run a monto carlo simulations. In fact, because losses ate the sum of some independent draws, its distribution is approximately normal wi th expected value and standard deviation given with previous folrmula:
```{r}
### CLT: The expected value and standard error of sum of 1000 loans
loss_per_foreclosure <- -200000
n <-  1000
sv <- n*(p*loss_per_foreclosure  + (1-p)*0)
sv
se <- sqrt(n)*abs(loss_per_foreclosure-0)*sqrt(p*(1-p))
se
```
The amount of x can be calculated to be added to each loan in the way that the expected value become 0 using the below equation:
lp+x(1-p)=0
x=-lp/1-p

This equation is the definition of expected value given a loss per foreclosure l with foreclosure probability p and profit x if there is no foreclosure (probability 1−p).
```{r}
### the profit x is equal to:
loss_per_foreclosure <- -200000
n <-  1000
p<- 0.02
x <- -(loss_per_foreclosure*p)/(1-p)
x #this is the interest rate of about 2%

### on a loan of 800000$, x is equal an interest rate of:
x/800000

```
this shows on average. But still there is 50% for losing money by bank. We have to choose an inteerst rate that makes is unlikely for bank to lose money. we have to choose inteerst somehow that be enough that bank does not loose money and also be compatetive with other banks. lets say that we want that the chance of loosing money by bank be one percent. what does x have to be now?

Pr (S<0) = 0.01

We note that S is approximately Normal. 
E[S] <- {lp+x(1-p)}n
SE[S] <- |x-l|sqrt(n.p.(1-p))

some mathematical tricks:
Pr(S<0) = Pr{(S-E[S])/ SE[S]  < -E[S]/SE[S]}
(S-E[S])/ SE[S] = standard normal random variable = z 
(base on z-score definition: Z <- x-mu/sigma)
Pr(Z < -E[S]/SE[S])

Now we put the actual value of E[S], SE[S]:
Pr(Z < -{lp+x(1-p)}n/(x-l)sqrt(n.p.(1-p)))=0.01

that it gives us z <- qnorm(0.01) that gives us the value of z for which Pr(Z<=z)=0.01  z=-{lp+x(1-p)}n/(x-l)sqrt(n.p.(1-p)))

solving for x:
x=-l(np-z(sqrt(np(1-p))))/{n(1-p)+zsqrt(np(1-p))}
...
x  #which is interest rate of 0.03 that still is good!

### Using CLT:
loss_per_closure*p + x*(1-p) #expected profit from loan
# it give the total expected profit of $2M

### running a Monto Caro simulation:
B <- 10000
profit <- replicate(B,{
  draws <- sample(c(l,x),n,replace=TRUE,prob=c(p,1-p))
  sum(draws)
})
mean(profit) #expected value of the profit over n loans (about $2M)  
mean(profit<0) # probability of losing money (about 1%)
```
## Key points: The big short
...
r <- 0.05
x <- 0.05*180000
loss_per_closure*p + x*(1-p) #Positive deposit earned by bank from each loan
```

### Probability of losing money
we can minimizing the chance of loosing money by increasing  the number of loans (n):
Pr(S<0)=Pr(Z < -E[x]/SE[x]): both E[x] and SE[x] are related to n.

E[x]=n.μ, SE[x]=√n*σ
based on these formula, -n.μ/√n*σ=-√n.μ/σ=-z

to find the value of n for which z is less than or equal to our desired value, we take z≤−√nμ/σ and solve for n:

n≥ z^2*σ^2/μ^2
for this value of n, we have the gurantee that we have the probability of 0.01 of loosing money.

Low of large numbers: when n is large, the average earning per loan converges to the expected earning mu.
...
A few month later one person create a Morgage company and instead of profiting, he lost money, why? they planed based on the following mathematical formula. by n large, we can minimize the standard error of our profit.
SE[X1+X2+...+Xn/n]=σ/√n
However, for being true this formula, the X must be independent draws. one person defaulting must be indepedent of other people defaulting. In extreme averageing an event over and over and in the case that the events be independent, we will get much higher standard error.
SE[X1+X2+...+Xn/n] = SE[nX1/n] = σ > σ/√n

for having a more realistic simulation, suppose ther is a b global event that effect everybody's morgages. these draws are not independent anymore:

The following Monte Carlo estimates the expected profit given an unknown probability of default 0.03≤p≤0.05, modeling the situation where an event changes the probability of default for all borrowers simultaneously. 
```{r}
...
mean(profit< -10000000) #probability of losing money over $10M! very high

library(tidyverse)
data.frame(profit_in_millions=profit/10^6) %>%
  ggplot(aes(profit_in_millions,..count..)) +
  geom_histogram(binwidth=5,col="black")
# as you can see, it is not normal at all!
```

## Exercise: Bank Earning
suppose that a bank gives 10,000 loans. the default rate is 0.03 and you lose 200,000$ in each foreclosure.  
```{r}
set.seed(1,sample.kind="Rounding")
n <- 10000
loss_per_foreclosure <- -200000
...
### what is the total money which bank lose? Using Monto Carlo simualtion
set.seed(1,sample.kind="Rounding")
B <- 10000
S <- replicate(B,{
  defaults <- sample(c(0,1),n,replace=TRUE,prob=c(1-p,p))
...
###what is the expected dvalue of S? assume that bank does not makes any money if the loan is paid.
n <- 10000
x <- 0
...
### in the previous part, bank did not earn any money and it was likely thta people dont pay their loans and bank lose money. Assume you give loans as amounts of $180,000. How much money do you need to make when people pay their loans so that you net loss be $0?

#rate?
#p*loss_per_foreclosure+x*(1-p)=0 #x total money that bank must earn
x <- -p*loss_per_foreclosure/(1-p)
x
rate <- x/180000
rate

### with the interest rate of previously calculated, bank still will lose money in 50% of situations. what must be interest rate for which the rate of losing money be 1 in 20. 
#based on the previous mathematical calculation, Pr (Z <= z)=0.05 Z=E[x]/SE[x]

z <- qnorm(0.05)
z
#z=-{lp+x(1-p)}*n/(x-l)sqrt(n.p.(1-p)))
x=-loss_per_foreclosure*(n*p-z*(sqrt(n*p*(1-p))))/{n*(1-p)+z*sqrt(n*p*(1-p))}
x   # the expected profit per loan
x/180000

```
## Assessment: Insurance companies
Insurance companies must make a decision how much to charge as premiums for policies given estimates of the probability that an individual will collect on that policy. 
An insurance company has a one-year life insurance policy that pays $150,000 for the event of death in a year. The premium for this policy is 1150 dollars for a women of age 50. suppose in the case of a claim for an event, the company forefit the premium and loses 150,000 dollars, and if theer is no claim, the company will earn 1150 dollars. The company plans to sell 1000 policicies to depographic. 
The death_prob data from dslabs includes some information about the estimated probability of death (prob) whithin 1 year for various ages and sexes
```{r}
library(tidyverse)
library(dslabs)
data(death_prob)
...
### using CLT, calculate the probaility of losing money by insurance company for 1000 insurers. 
pnorm(0,Ev_all,SE_all)
```

### Calculation the profitable premium for male of 50 years old:
```{r}
### what is the death probaility of death of a 50 years old female:
p_male <- death_prob %>%
...
### suppose that the company wants to have the expected value of profit of 700,000$ for insuring 1000 people who are male of 50 years old. 
#Ev_male <- n(ap+b(1-p))  #b: premium   #a:150,000$ for death
Ev_male <- 700000
n <- 1000
a_male <- -150000
...
### what is the probbaility of losing money by insurance company for insuring 1000 male of 50 years old?
pnorm(0,Ev_male,SE_all_male)
```

### insurance artes:
Insurance companies calculate the premimum and ... based in the rencet probability of death. However, it is possible due to an event, the probaility of death increases and company risk considerable amount of lose. 
In the below situation, there is lethal pandemic situation and increases the probability of death for 50 years old to 0.015. because company was unable to predict this outbreak, it sold 1000, 150000 dollars  life insurance policies as premium of $1150. 
...
### for the death probability of p <- seq(.01, .03, .001), how much is the lowest death probbaility for which the chance of losing money exceed 90%?
...
### for the death probaility of p <- seq(.01, .03, .0025), what is the lowest death probability for which the chance of losing more than $1 million exceeds 90%?
p <- seq(.01, .03, .0025)
...
data.frame(p,p_lose_1M) %>%
  ...
  min()
```
### Using Monto-Carlo simualtion:
define a sampling model that has the following properties:
total profit over 1,000 loans with probability of claim p_loss = .015, loss of -$150,000 on a claim, and profit of $1,150 when there is no claim.
```{r}
### sampling model:
n <- 1000
loss <- -150000
...
set.seed(25, sample.kind = "Rounding")  # based on question: 25
X <- sample(c(loss,profit),n,replace=TRUE,prob=c(p_loss,1-p_loss)) #loss:p, corresponds 1
sum(X)/10^6
```
```{r}
### Monto-carlo Simulation: 
#run the monto-carlo simulation with 10,000 repetition to simulate the range of profits/losses over 1,000 loans
...
mean(S_lose < -1000000) # it means more than $1 M losing money (Be carful about negative numbers)
```
### Assesment: Insurance due to pandemic 
suppose that there is high demand for insuring due to a pandemic. The insurance company looking for a primium that ensure that the  probaility of lose of the company is less than 5%. assume that the death rate is 0.015.

```{r}
### calculate the premium required for 50% chance of losing money for 1000 policies. probbaility of loss is 0.015, and loss per cliam is -150,000$.
p <- 0.015
l <- -150000
n <- 1000
# x ? (premium)
...
### expected profit per policy for this rate?
Ev <- p*l + (1-p)*x
Ev

### expected profit for all 1000 policies?
Ev_all <- n * (p*l + (1-p)*x)
Ev_all

### based on new premium x, run a monto carlo simulation with 10,000 repetition. what the probability of losing money for 1000 policies, death probaility of 0.015, claim of 150,000$, set.seed of 28?
B <- 10000
library(tidyverse)
set.seed(28, sample.kind = "Rounding")
S_losing <- replicate (B,{
  draws<- sample(c(l,x),n,replace=TRUE,prob=c(p,1-p))
  sum(draws)
})

Pr_losing <- mean(S_losing <0)
Pr_losing

### suppose the insurance company does not know id the pandemic deatn rate is stable or not. Therefore suppose p randomly change between -0.01, and 0.01. calculate the profit from premium x which previously calculated? loss per claim is 150,000$, B=10,000, set.seed od 29. 

### how much is the expected value over 1000  policies?
p <- 0.015

B <- 10000
set.seed(29, sample.kind = "Rounding")
S_new_losing <- replicate (B,{
  p_new <- p+sample(seq(-0.01,0.01,length=100),1)
  draws_new<- sample(c(l,x),n,replace=TRUE,prob=c(p_new,1-p_new))
  sum(draws_new)
})
...
### what is the probbaility of losing money
mean(S_new_losing<0)

### what is the probaility of losing more than $1M?
mean(S_new_losing > -10^6)
